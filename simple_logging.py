...
class SimpleLoggingManager:
    ...
    def setup_logging(self, config_dict: Dict[str, Any], verbose_console: bool = True, 
        ...
        # Create file handler
        # Use output directory from config instead of default logs directory
        # Check for output directory in the config
        output_dir = config_dict.get('output_directory', None)
        if not output_dir:
            # Fallback to the old structure
            output_dir = config_dict.get('output', {}).get('directory', 'logs')
